...
void	Bureaucrat::executeForm(AForm &toExecute){
	if (toExecute.execute(*this)){
		std::cout << *this << " executed form " << 
		toExecute.getName() << std::endl;
	}
}
...
void Bureaucrat::signForm(AForm &toSign){
	if (toSign.getGradeToSign() >= this->getGrade() 
		&& !toSign.getSigned()){
		toSign.beSigned(*this);
		std::cout << *this << " signed " << toSign.getName()
			<< std::endl;	
	}
	else if (toSign.getSigned()){
		std::cout << *this << " couldn't sign " << toSign.getName()
			<< " because it was already signed" << std::endl;
	}
	else {
		std::cout << *this << " couldn't sign " << toSign.getName()
			<< " because it doesn't have the authority" << std::endl;
	}
}
